...
function M.set(ctx)
  -- [GENERAL SETTINGS]
  -- -----------------------------------------------------------------------
  M.project_root = ctx.project_root
  ...
  if is_windows then
    M.browser_cmd = ctx.browser_cmd or "start"
  else
  ...
  -- [DOXYGEN]
  -- ------------------------------------------------------------------------
  M.doxygen_filetypes = ctx.doxygen_filetypes
      or {
        "c",
        "cpp",
        "cs",
  ...
  M.doxygen_html_file = ctx.doxygen_html_file
      or utils.os_path "html/index.html"

  -- auto setup
  -- Defaults: clone the repo into 'doxygen'.
  M.doxygen_clone_config_repo = ctx.doxygen_clone_config_repo
  ...
  M.typedoc_clone_config_repo = ctx.typedoc_clone_config_repo
      or "https://github.com/Zeioth/vim-typedoc-template.git"
  M.typedoc_clone_to_dir = ctx.typedoc_clone_to_dir
      or M.typedoc_clone_config_repo:match ".+/(.-)%.git" -- URL's repo name
  M.typedoc_clone_cmd_post = ctx.typedoc_clone_cmd_post
      or
  ...
  M.jsdoc_clone_config_repo = ctx.jsdoc_clone_config_repo
      or "https://github.com/Zeioth/vim-jsdoc-template.git"
  M.jsdoc_clone_to_dir = ctx.jsdoc_clone_to_dir
      or M.jsdoc_clone_config_repo:match ".+/(.-)%.git" -- URL's repo name
  M.jsdoc_clone_cmd_post = ctx.jsdoc_clone_cmd_post
      or
